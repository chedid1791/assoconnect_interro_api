...
function Get-AssoConnectData {
    ...
    $HasMoreData = $true
    $ItemsTotaux = 0
    $Groups = Import-Csv -Path "$RepData\Groups_avances.csv" -Delimiter ";"

    $IndexGroups = @{}

    foreach ($Group in $Groups) {
        $IndexGroups[$Group.Id] = $Group
    }

Write-Host "Fin de la procédure d'indexation des groupes." -ForegroundColor Green
            ...
            Foreach ($Contact in $Response.'hydra:member') {
                If ($contact.Type -eq "PERSON") {
                    ...
                    foreach ($Relation in $Affiliation) {
                        $toto = ($Relation.organization -split '/')[-1]
                        If ($toto -in $IndexGroups.Keys) {
                            $GA += $toto
                        }
                     }
